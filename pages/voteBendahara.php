...
if(isset($_POST['selanjutnya'])){
	if(!isset($_POST['kandidatBendahara'])){
	...
	$_SESSION['chosenBendahara'] = $_POST['kandidatBendahara'];
			...
			<form action="" method="post">
				<nav>
					<div class="nav nav-tabs gap-1" id="nav-tab" role="tablist">
						<button class="nav-link active fs-4" id="nav-home-tab" data-bs-toggle="tab" data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home" aria-selected="true">Bendahara</button>
										...
										<div class="container-fluid">
											<div class="row row-cols-1 gap-2">
												<div class="vote-btn d-flex justify-content-center text-center position-relative p-0" id="votee">
													<input type="radio" name="kandidatBendahara" class="position-absolute opacity-0" id="b<?= $row['id'] ?>" value="<?= $row['nama'] ?>">
													<label for="b<?= $row['id'] ?>" class="color-bg-primary text-light rounded w-100 p-2" style="cursor: pointer;" >Vote</label>
												</div>
